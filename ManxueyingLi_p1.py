...
def with_linear_conflict(curr_state):#return manhattan dis + linear conflict
    ...
    # print(row_curr,row_goal,col_curr,col_goal)
...
def eval_func(node):#return the value of evaluation function w/ method given
    if which_eval_func_to_use == "only_manhattan":
       return node.path_cost + Manhattan_dist(node.state)
        ...
        return node.path_cost + with_linear_conflict(node.state)
...
def astar(state):
    ...
    while frontier:#if frontier is not empty
        ...
        for child in node.expand(state): #expand the node which is loop through the list of all possible states(child node)
            if tuple(child.state) not in explored and (eval_func(child), child.state) not in frontier.queue: #new state
                frontier.put((eval_func(child), child))# add to frontier
                # print(state_str(node.state),"\n",eval_func(node),node.path_cost)
                # print("lc",with_linear_conflict(node.state),end = "\n++++++++++++++++++++++++\n")
                count += 1
            elif (eval_func(child), child.state) in frontier.queue: #check if it can update state that already in the the frontier
                this = frontier.get()
                if eval_func(child) < int(this[0]):#if f(n) is smaller
                    frontier.put(child) #add to frontier
                    count += 1
...
f = open("input1.txt",'r') 
#output file name
f_outA = open("Output1_A.txt",'w') #for output A (only manhattan)
f_outB = open("Output1_B.txt",'w') #for putput B (w/ linear conflict)
